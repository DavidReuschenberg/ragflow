...
from api.utils.log_utils import get_project_base_directory, init_root_logger
...
from rag.app import (audio, book, email, laws, manual, naive, one, paper,
                     picture, presentation, qa, resume, table, tag, xml)
...
from rag.settings import (DOC_BULK_SIZE, DOC_MAXIMUM_SIZE,
                          EMBEDDING_BATCH_SIZE, PAGERANK_FLD,
                          SVR_CONSUMER_GROUP_NAME, TAG_FLD, get_svr_queue_name,
                          get_svr_queue_names, print_rag_settings)
...
async def build_chunks(task, progress_callback):
    ...
    try:
        ...
        logging.info(f"DEBUG: About to load binary from MinIO - bucket='{bucket}', name='{name}'")
        binary = await get_storage_binary(bucket, name)
        logging.info(f"DEBUG: Binary loaded from MinIO - type={type(binary)}, is_none={binary is None}, size={len(binary) if binary else 'N/A'}")
        ...
        logging.error(f"DEBUG: Exception loading binary from MinIO - bucket='{bucket}', name='{name}', error={e}")
    ...
    try:
        logging.info(f"DEBUG: About to call chunker with binary type={type(binary)}, is_none={binary is None}")
