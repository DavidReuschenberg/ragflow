...
<i>不符合上述规则的文本行将被忽略。</i>
`,
      xml: `<p>支持的文件格式为<b>XML</b>。</p><p>
      此方法专为具有层次结构的结构化 XML 文档设计，例如包含成本计算的司法或法律文档。
      </p><p>
      解析器通过识别段落、章节、程序和计算等关键元素作为自然块边界来尊重 XML 结构。
      </p><p>
      您可以配置目标元素以专注于 XML 文档的特定部分。常见元素包括：段落、章节、文章、项目、条目、成本、计算、费用、程序、诊断、治疗。
      </p><p>
      启用"保留 XML 结构"以在生成的块中保持元素之间的层次关系。
      </p>`,
